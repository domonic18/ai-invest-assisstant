...
import { useGlobalIndexHistory, useGlobalIndices, useMarketIndices } from '@/hooks/useMarket'
...
function GlobalCard({ quote, onOpen }: GlobalCardProps) {
  const meta = TAG_META[quote.indexCode] ?? { tag: '全球', color: 'blue' }
  const isBond = BOND_ORDER.includes(quote.indexCode)
  ...
  return (
    <MacroCard
      ...
      trend={quote.trend}
      onClick={() => onOpen(quote.indexCode)}
      footer={
        <>
...
export function MacroMonitor() {
  ...
  // 2s10s 利差为衍生指标，由 2Y/10Y 历史序列推算卡片数值
  const { data: spreadHistory } = useGlobalIndexHistory(SPREAD_CODE, 12)

  const openDetail = (code: string) => navigate(`/index/${code}`)
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="text-xs text-gray-600">
          ...
          数据源：新浪 / 东方财富 / Tushare / Yahoo / 日本财务省 / CME · 点击卡片查看指数详情
        </div>
      </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
          ...
          {EQUITY_ORDER.map((code) => {
            ...
            return <GlobalCard key={code} quote={quote} onOpen={openDetail} />
          })}
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
          ...
          <MacroCard
            ...
            trend={(spreadHistory ?? []).map((p) => p.close)}
            onClick={() => openDetail(SPREAD_CODE)}
            footer={
              <>
