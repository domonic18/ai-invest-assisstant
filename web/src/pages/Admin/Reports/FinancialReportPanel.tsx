...
const FILE_TYPE = 'financial_report'
...
export function FinancialReportPanel() {
  ...
  const [params, setParams] = useState({ stockCode: '', page: 1, pageSize: PAGE_SIZE.table })
  const [modalOpen, setModalOpen] = useState(false)
  const [editing, setEditing] = useState<AdminReport | null>(null)

  const { data, isLoading } = useAdminReports({ ...params, fileType: FILE_TYPE })
  ...
  const handleSubmit = async (values: ReportFormValues) => {
    ...
    try {
      if (editing) {
        ...
        message.success('财报已更新')
      } else {
        await createMutation.mutateAsync(payload)
        message.success('财报已创建')
      }
      setModalOpen(false)
    } catch (err) {
  ...
  const columns: ColumnsType<AdminReport> = [
    ...
    { title: '报告日期', dataIndex: 'reportDate', key: 'reportDate', render: (v: string | null) => formatDate(v) },
