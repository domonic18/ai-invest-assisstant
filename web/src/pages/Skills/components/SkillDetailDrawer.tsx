...
import { Descriptions, Drawer, Segmented, Skeleton, Tag, Typography } from 'antd'
import { useEffect, useState } from 'react'

import { MarkdownText } from '@/components/common/MarkdownText'
import { useSkillDetail, useSkillFiles } from '@/hooks/useSkills'
...
export function SkillDetailDrawer({ skillId, onClose }: SkillDetailDrawerProps) {
  ...
  return (
      ...
      title={
        skill ? (
          <span className="flex items-center gap-2">
            ...
            {!skill.isBuiltin && <Tag bordered={false}>自定义</Tag>}
          </span>
        ) : (
          '技能详情'
        )
    ...
    >
      {skill && (
        <>
          <div className="shrink-0 border-b border-[#23262d] px-5 py-3">
            <Descriptions
              column={2}
              size="small"
              items={[
