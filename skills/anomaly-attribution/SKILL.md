...
allowed-tools: search_news_by_date, search_news, get_dragon_tiger, get_stock_fund_flow, get_sector_fund_flow, get_stock_technical, persist_sector_anomaly_attribution, persist_stock_anomaly_attribution, search_knowledge_base
---

# 异动 AI 归因

## 描述
规则算子已回答「发生了什么异动」（趋势拐点 + 量价维度，趋势事实 trend_facts 已预计算，见异动检测服务）；本 Skill 只回答「为什么」：先把异动放回趋势位置（通道归属 / 拐点类型 / 量能确认）中考量，再调用证据工具核实消息面 / 资金面 / 龙虎榜，对每一条异动给出归因分类（可修正规则分类）与 1-2 句归因摘要。证据只进入摘要文本，不得编造检测数据之外的事实。最终回复必须且只能是符合「输出 Schema」的 JSON 对象。
...
个股域（stock）：
```json
{"items": [{"stock_code": "6 位代码", "category": "breakout|breakdown|acceleration|pullback", "summary": "归因摘要（1-2 句）"}]}
```
- `category` 语义：板块 resonance=趋势共振（多主线同向）、rotation=轮动补涨；个股 breakout=突破拐点（带量收复关键均线或带量创阶段新高，且以周线 M60 之上为前提——trend_facts.above_weekly_ma60=false 时规则层已不判突破）、breakdown=破位下行（放量跌破关键均线的风险拐点）、acceleration=趋势内加速、pullback=下跌反抽。
- `items` 必须覆盖输入清单中的全部标的，禁止新增清单外的标的；summary 必须简体中文。
- 引用契约：summary 涉及趋势/拐点判断时，内嵌标准引用 `《趋势理论》第N集 MM:SS（所在章节标题）`（集数与时间码只能来自随提示注入的 `skills/anomaly-attribution/methodology.md` 尾注或 `search_knowledge_base` 检索结果）；确无适用方法论时，摘要末尾另起一行输出 `> 知识库佐证：无适用方法论`。
...
- `get_stock_technical(stock_code, trade_date?)`: 个股技术面预计算概要（趋势通道、均线结构、量能状态）——个股趋势位置证据，与清单中 trend_facts 同口径，用于交叉核实。
...
2. 板块域：对强度靠前的板块调 `get_sector_fund_flow` 核实主力资金；个股域：对上榜/高换手个股调 `get_stock_technical` 交叉核实趋势位置，再调 `get_dragon_tiger` 与 `get_stock_fund_flow` 核实资金与龙虎榜证据。
...
- 涉及趋势/拐点判断的摘要须按引用契约内嵌《趋势理论》引用或以「> 知识库佐证：无适用方法论」结尾；禁止编造集数与时间码。
