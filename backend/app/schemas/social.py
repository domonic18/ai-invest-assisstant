...
class AsrConfigResponse(CamelModel):
    """GET /admin/social/asr-config 响应（masked 视图，密钥只回脱敏串）。"""
...
class AsrConfigUpdateRequest(CamelModel):
    """PUT /admin/social/asr-config 请求（apiKey write-only：None/空串=保留原值）。"""
...
class AsrConfigTestResponse(CamelModel):
    """POST /admin/social/asr-config/test 响应：连接测试结果。"""
