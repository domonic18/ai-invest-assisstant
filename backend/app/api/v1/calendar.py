...
@router.get("/events", response_model=list[NewsCalendarEventResponse])
async def list_news_calendar_events(
...
) -> list[NewsCalendarEventResponse]:
    ...
    return [NewsCalendarEventResponse.model_validate(e) for e in events]


@router.get("/events/upcoming", response_model=list[NewsCalendarEventResponse])
async def list_upcoming_news_calendar_events(
    session: Annotated[AsyncSession, Depends(get_db)],
