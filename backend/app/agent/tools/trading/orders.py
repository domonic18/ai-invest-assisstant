...
def make_place_order_tool(agent_key: str) -> BaseTool:
    """构建下单工具（闭包绑定 agent_key）。"""
    ...
    async def place_paper_trade_order(
        ...
        async with AsyncSessionLocal() as session:
            try:
                agent = await get_agent(session, agent_key)
    ...
    return place_paper_trade_order


def make_cancel_order_tool(agent_key: str) -> BaseTool:
    """构建撤单工具（闭包绑定 agent_key）。"""
    ...
    async def cancel_paper_trade_order(cl_ord_id: str) -> dict[str, Any]:
        ...
        async with AsyncSessionLocal() as session:
            try:
                account = await ensure_agent_account(session, agent_key)
    ...
    return cancel_paper_trade_order
