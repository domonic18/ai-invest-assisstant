...
class TestGetStockQuote:
    ...
    async def test_eod_fallback_when_live_expired(self) -> None:
        ...
        with (
            ...
            patch.object(stock_service, "cache_mget", mget),
        ):
    ...
    async def test_all_miss_returns_empty_quote_not_none(self) -> None:
        ...
        with (
            ...
            patch.object(stock_service, "cache_mget", AsyncMock(return_value=(None, None))),
    ...
    async def test_redis_unavailable_falls_back_to_kline(self) -> None:
        """回归：Redis 不可达（SCF 网络隔离）时快照降级 miss，回退日 K 而非 500。"""
        session = AsyncMock()
        failing = AsyncMock()
