"""THS (10jqka) sector fund flow collector — 板块资金流同花顺渠道。

同花顺行业/概念资金流（data.10jqka.com.cn/funds/）与东财口径不同：
无板块代码（以行业/概念名代替）、无超/大/中/小单拆分（写 NULL）。
通过 akshare.stock_fund_flow_industry / stock_fund_flow_concept 获取
（hexin-v 签名由 akshare 内置处理）。写入与东财采集器相同的
capital_fund_flow_sector 表。
"""
...
class ThsSectorFundFlowCollector(BaseSectorFundFlowCollector):
    """同花顺板块资金流向采集器，写入 capital_fund_flow_sector。"""
    ...
    async def collect(
        ...
        fetcher = (
            ...
            else ak.stock_fund_flow_concept
        )
