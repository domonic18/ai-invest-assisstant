...
- **query 参数名跟随后端 FastAPI 签名（snake_case），不随 body 的 camelCase**：FastAPI 按函数签名参数名解析 query string，不做 camel 转换；前端发 camel 键会被静默忽略并回退默认值（2026-09-07 market.ts tradeDate 失配致复盘页日期切换全失效）
...
**ask_user 问题卡**（交互确认底座，arch/05 §5.3）：skill 在写操作前需用户决策时调 `ask_user(question, options, default?)` 工具——返回值携带 `__question__` 标记结束本轮，runs 流端点检测后经 SSE `custom` 事件下发 `{"type":"question",...}`，前端 `QuestionCard` 组件渲染选项按钮，点击即"我选择：{label}"作为新消息续跑线程。定时任务路径无此交互，不使用 ask_user。
