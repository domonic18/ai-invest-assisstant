...
3. **归因证据自由**：归因 LLM 经工具引用新闻 / 公告 / 龙虎榜作为证据，但证据只出现在归因文本中，不落检测表、不影响强度分。
4. **趋势上下文一票否决方向感**：同样的量价命中，多头趋势中是「启动 / 加速」，空头趋势中是「反抽」，按分类降权而非等同呈现。

## 2. 板块异动检测

数据源为 `quote_sector_daily`（行业 + 概念收盘快照），盘后对全部板块跑三维判定：

| 维度 | 规则 | 说明 |
|------|------|------|
| 涨跌幅偏离 | `abs(change_pct) ≥ 2%` | 价格触发主维度 |
| 量能异常 | `amount ≥ 5 日均额 × 2` | 基线不足（快照积累 < 5 交易日）时该维度跳过，其余维度正常判定 |
...
- **证据工具**：资讯查询（电报 / 新闻文档）、公告、龙虎榜只读查询——证据仅用于归因文本引用。
